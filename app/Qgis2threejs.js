...
(function () {
  ...
  gui.showQueryResult = function (point, layer, obj, show_coords) {
    ...
    if (e) {
      ...
      let wikidataId = null; // Store Wikidata ID if available
      ...
      if (layer && layer.properties.propertyNames !== undefined) {
        ...
        for (var i = 0, l = layer.properties.propertyNames.length; i < l; i++) {
          ...
          var propertyName = layer.properties.propertyNames[i];
          var propertyValue = obj.userData.properties[i];

          // Skip null or undefined values
          if (propertyValue === null || propertyValue === undefined) continue;

          // Convert Wikidata ID into a clickable link
          var propertyContent =
            propertyName === "wikidata" && propertyValue.startsWith("Q")
              ? `<a href="https://www.wikidata.org/wiki/${propertyValue}" target="_blank">${propertyValue}</a>`
              : propertyValue;

          row.innerHTML = `<td>${propertyName}</td><td>${propertyContent}</td>`;
          e.appendChild(row);

          // Store Wikidata ID
          if (propertyName === "wikidata" && propertyValue.startsWith("Q")) {
            wikidataId = propertyValue;
          }
        }

        // Add button to show image if Wikidata ID is available
        if (wikidataId) {
          ...
          button.onclick = () => fetchWikidataImage(wikidataId);
    ...
    // Function to fetch the image from Wikidata
    async function fetchWikidataImage(qid) {
      ...
      try {
        ...
        if (entity.claims.P18) {
          ...
          alert("No image found on Wikidata.");
        }
      } catch (error) {
    ...
    // Function to show the image in a centered pop-up
    ...
    // Add the pop-up HTML to the page if not already present
    if (!document.getElementById("image-popup")) {
      document.body.insertAdjacentHTML(
        "beforeend",
        `
			<div id="popup-overlay" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0, 0, 0, 0.5); z-index:999;"></div>
			<div id="image-popup" style="display:none; position:fixed; top:50%; left:50%; transform:translate(-50%, -50%); background:white; padding:20px; border-radius:10px; box-shadow:0 4px 8px rgba(0,0,0,0.2); z-index:1000; text-align:center;">
				<span onclick="document.getElementById('image-popup').style.display='none'; document.getElementById('popup-overlay').style.display='none';" style="cursor:pointer; float:right; font-size:18px;">✖</span>
				<img id="popup-image" src="" style="max-width:100%; max-height:80vh; display:block; margin:10px auto;">
			</div>
		`
      );
    }
